...
if str(_DNCNN) not in sys.path:
    sys.path.insert(0, str(_DNCNN))
...
def _cnn_mp_init(pack: dict) -> None:
    ...
    sys.path.insert(0, pack["dncnn_root"])
    import torch as _t
...
def _run_one_split(args: argparse.Namespace, *, split: str, clear_outputs: bool = True) -> None:
    ...
    plan = resolve_viz_inference_plan(
        args, repo=_REPO, data_root=data_root, data_tag=tag, nn_dir=_DNCNN
    )
    ckpt_paths = list(plan.ckpt_paths)
    is_ensemble = plan.mode == "ensemble"
    ckpt_path = ckpt_paths[0]

    maybe_sync_split_from_runs_config(args, runs_dir=plan.config_dir, log_prefix="[dncnn-viz]")
    ...
    if allowed_keys is None:
        print(
            f"[dncnn-viz] split={split} -> {describe_split_for_log(split, cv_folds=int(args.cv_folds), cv_fold=int(args.cv_fold))} "
            f"(no ztest5 manifest; using single-dataset 8:2 split)",
            flush=True,
        )

    _out_base = Path("output") / "dncnn" / tag
...
def main() -> int:
    p = argparse.ArgumentParser(
        description="cnn: DnCNN visualization/export (invoked by repo-root visualize_data.py dncnn).",
        epilog="Minimal usage: python visualize_data.py dncnn --data-root data1\n"
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Weights dir; if omitted, prefer DnCNN/output/<dataset>/runs, then repo output/<dataset>/runs, etc.",
    )
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Image output dir; default output/dncnn/<dataset>/image",
    )
    p.add_argument(
        "--result-dir",
        type=str,
        default=None,
        help="Result output dir; default output/dncnn/<dataset>/result",
    )
    ...
    print(
        f"[dncnn-viz] total {time.perf_counter() - t0:.2f}s (split={args.split} export_workers={args.export_workers})",
        flush=True,
    )
    return 0
