"""K-fold checkpoint inference for DnCNN, TraMagNet, and UNet-only ablation."""
...
NN_INFER_METHODS = frozenset({DNCNN_METHOD, UNET8_METHOD, OUR_METHOD})
...
SIX_PANEL_HEADINGS = PANEL_HEADINGS
...
def _zscore_using_reference_stats(y: np.ndarray, reference_ref: np.ndarray, *, eps: float = 1e-6) -> np.ndarray:
    c = np.asarray(reference_ref, dtype=np.float64).ravel()
...
def _normalize_and_slice_rows_for_plot(
    ...
    *,
    reference_ref: np.ndarray,
    reference_ref1: np.ndarray | None,
    dual: bool,
    plot_sl: slice,
    ...
    for i, row in enumerate(rows):
        ...
        if dual:
            ...
            c0 = reference_ref
            c1 = reference_ref1 if reference_ref1 is not None else reference_ref
            ...
            ny = _normalize_panel_for_plot(reference_ref, y, match_noisy_scale=match_noisy)[plot_sl]
...
def _resolve_gan5_inference_plan(
    ...
    *,
    ...
    """Resolve TraMagNet K-fold weights under ``output/<pool>/runs`` or an explicit ``--runs-dir``."""
...
class TraMagNetCvInferer:
    ...
    def _preprocess_single_pair(
        ...
        t = self._torch
        clean = t.tensor(c_r, dtype=t.float32).unsqueeze(0)
        noisy = t.tensor(n_r, dtype=t.float32).unsqueeze(0)
        if self.pre_cfg.match_noisy_scale:
            noisy = _affine_match_mean_std(noisy, clean, eps=float(self.pre_cfg.eps))
        reference_phys = clean.clone()
        if self.pre_cfg.zscore_using_reference:
            mu, sig = _mean_std(clean, eps=float(self.pre_cfg.eps))
            clean = _zscore(clean, mu, sig)
            noisy = _zscore(noisy, mu, sig)
        clean = t.nan_to_num(clean, nan=0.0, posinf=0.0, neginf=0.0)
    ...
    def infer_dual(
        ...
        *,
        ...
        def _affine_match_mean_std(
            noisy: "torch.Tensor", clean: "torch.Tensor", *, eps: float = 1e-6
        ) -> "torch.Tensor":
...
def _resolve_cnn3_inference_plan(
    ...
    *,
    ...
    prefer = str(getattr(viz_ns, "ckpt", "last"))
    bases: list[Path] = []
    if getattr(viz_ns, "dncnn_runs_dir", None):
        bases.append(Path(str(viz_ns.dncnn_runs_dir)))
    bases.extend(
        [
            plan.config_dir.parent / "runs",
            plan.config_dir,
...
class DnCNNCvInferer:
    ...
    def __init__(
        ...
        *,
        ...
        viz_ns = SimpleNamespace(
            runs_dir=getattr(args, "dncnn_runs_dir", None),
            ckpt=str(getattr(args, "ckpt", "last")),
            cv_ensemble_folds=int(getattr(args, "cv_ensemble_folds", 5)),
            no_cv_ensemble=bool(getattr(args, "no_cv_ensemble", False)),
        )
    ...
    def infer_single(
        ...
        *,
        ...
        try:
            ...
            print(
                f"[WARN] DnCNN baseline-cv inference failed ({noisy_path.name}); DnCNN panel uses noisy placeholder: {e}",
                flush=True,
            )
            return np.asarray(fallback, dtype=np.float64).copy()
