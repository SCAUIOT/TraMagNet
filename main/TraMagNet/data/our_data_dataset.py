...
class OurDataConfig:
    """
    Data directory (consistent with the current project); pairing and parsing are unified via
    ``data_common`` + ``data*/read_official``.
    """
    ...
    # Affine: align segment-wise mean/std of noisy to reference (remove global gain/bias offset); does not guarantee pointwise noisy≈reference.
