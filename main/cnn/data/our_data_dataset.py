...
from data_common.flat_pairing import axis_from_clean_path, sample_id_from_clean_path
...
class OurDataConfig:
    ...
    # Affine: align segment mean/std of noisy to clean (remove global gain/bias); does not guarantee pointwise noisy≈clean.
    match_noisy_scale_to_clean: bool = False
    zscore_using_clean: bool = False
...
class OurDataDataset(Dataset):
    """
    Output item:
      - dict(clean=(1,T), noisy=(1,T), key=str, mask=(T,))
    """
    ...
    def __init__(self, cfg: OurDataConfig) -> None:
        ...
        self.cfg = cfg
        root = Path(cfg.root)
        clean_dir = root / cfg.clean_subdir
        ...
        for sp in specs:
            sid = sample_id_from_clean_path(sp.clean_path, data_root=root)
            if not sid:
                continue
            axis = axis_from_clean_path(sp.clean_path, data_root=root)
            pairs.append((sid, axis, str(sp.clean_path), str(sp.noisy_path), int(sp.value_column)))

        if not pairs:
            raise RuntimeError(
                f"No paired samples under {clean_dir} / {noisy_dir} for band={cfg.band!r}. "
                f"Ensure matching sample{{i}}.txt exist in both clean_signal/ and noise_signal/."
            )
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor | str]:
        ...
        norm_cfg = config_from_dataset_flags(
            match_noisy_scale_to_clean=bool(self.cfg.match_noisy_scale_to_clean),
            zscore_using_clean=bool(self.cfg.zscore_using_clean),
            normalization=self.cfg.normalization,
            eps=float(self.cfg.eps),
        )
        clean, noisy = normalize_pair(clean, noisy, norm_cfg)

        key = Path(n_fn).stem
        if vcol != 2:
            key = f"{key}__vcol{vcol}"
        return {"clean": clean, "noisy": noisy, "key": key, "mask": mask}
