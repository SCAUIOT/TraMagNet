...
def _rich_make_panel(
    *,
    ...
    if mse_nc0 is not None:
        ...
        line2.append("First-batch MSE(noisy,clean): ", style="dim")
    ...
    if diag:
        ...
        diag_block.append(
            ...
            f"MSE(noisy,clean)={mse_c * ls_log:.6f}\n",
            style="white",
        )
        diag_block.append(
    ...
    if diag:
        ...
        lines.extend(
            [
                ...
                f"    MSE(denoised, clean) = {mse_b * ls_log:.6f}",
                f"    MSE(noisy, clean) = {mse_c * ls_log:.6f}",
                "    Dimensionless ratios: A/C = MSE(denoised,noisy)/MSE(noisy,clean), "
                "B/C = MSE(denoised,clean)/MSE(noisy,clean), A/B = MSE(denoised,noisy)/MSE(denoised,clean)",
...
def evaluate_identity_mse(
    ...
    - B = MSE(denoised, clean): should decrease with training.
    - C = MSE(noisy, clean): identity baseline (error if copying noisy).
    ...
    for batch in loader:
        ...
        sb += float(masked_mse(denoised, clean, mask).item())
        sc += float(masked_mse(noisy, clean, mask).item())
        n += 1
    d = max(1, n)
...
def _add_dncnn_arguments(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument(
        ...
        type=float,
        default=0.0,
        dest="clean_l1_weight",
        help="Add coefficient × L1(denoised, clean) beyond noise supervision; 0 off. Try 0.05~0.1 to curb identity collapse.",
    )
    p.add_argument(
        "--grad-match-weight",
        type=float,
    ...
    p.add_argument(
        ...
        help=(
            ...
            "Enable with --zscore-using-clean."
        ),
    )
    p.add_argument(
        "--data-sanity",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="At startup print first train batch MSE(noisy,clean) (same definition as diag C).",
    )
    p.add_argument(
        "--log-style",
        type=str,
...
def train_one_fold(args: argparse.Namespace, fold: int, out_path: Path) -> None:
    ...
    if log_style == "full":
        if mse_nc0 is not None:
            print(
                f"[data sanity] first batch MSE(noisy, clean) = {mse_nc0:.6f} | "
                f"noisy aligned to clean amplitude = {args.match_noisy_scale} | "
                f"z-score using clean segment = {args.zscore_using_clean}",
                flush=True,
            )
        print(
            "[task] Supervision: predict noise pred_noise ≈ (noisy-clean); "
            "validation: masked_loss(denoised, clean).",
            flush=True,
        )
        if float(args.clean_l1_weight) > 0 or float(args.grad_match_weight) > 0:
            print(
                f"[extra losses] clean L1 weight = {args.clean_l1_weight}, "
        ...
        if float(args.clean_l1_weight) > 0 or float(args.grad_match_weight) > 0:
            print(
                f"[train] extra losses: clean_l1={args.clean_l1_weight} grad_match={args.grad_match_weight}",
                flush=True,
            )
    else:
        parts = [
        ...
        if mse_nc0 is not None:
            parts.append(f"MSE(noisy,clean) first_batch={mse_nc0:.4g}")
        parts.extend(
            [
                f"amp_align={args.match_noisy_scale}",
                f"zscore={args.zscore_using_clean}",
                f"out_dir={args.out_dir}",
            ]
        )
        if float(args.clean_l1_weight) > 0 or float(args.grad_match_weight) > 0:
            parts.append(f"extra_l1={args.clean_l1_weight} grad_match={args.grad_match_weight}")
    ...
    try:
        for ep in range(start_ep, int(args.epochs) + 1):
            ...
            for batch in train_loader:
                ...
                # Explicit noise supervision; masked_loss(denoised, clean) is the same objective family (mse / l1 / huber)
                noise_tgt = noisy - clean
                ...
                clw = float(args.clean_l1_weight)
                if clw > 0:
                    loss = loss + clw * masked_l1(denoised, clean, mask)
                gmw = float(args.grad_match_weight)
                if gmw > 0:
                    loss = loss + gmw * masked_temporal_mse(denoised, clean, mask)
