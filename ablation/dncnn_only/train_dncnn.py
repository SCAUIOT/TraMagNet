...
def _run_dncnn_training(args: argparse.Namespace, *, fold: int, out_path: Path) -> None:
    ...
    if pooled:
        ...
        print(
            f"[DnCNN-only ablation] pooled data: {[t for t, _ in entries]} manifest="
    ...
    migrate_job_root_checkpoints_into_runs(out_path, log_prefix="[DnCNN-only ablation]")
