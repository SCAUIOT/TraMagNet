"""
UNet-only ablation training: single-channel ablation (no DnCNN, no latent z).

Input is ``noisy`` only (``UNetBackbone1D(in_ch=1)``).
Defaults aligned with UNet-only ablation / TraMagNet data134 5-fold 2000 epochs.
"""
...
from models.unet import UNetSingle  # noqa: E402
from models.unet_loss import mse_time_frequency_loss, supervised_unet_loss  # noqa: E402
...
def train_one_fold(args: argparse.Namespace, fold: int, out_path: Path) -> None:
    _run_unet_training(args, fold=fold, out_path=out_path)
...
def _run_unet_training(args: argparse.Namespace, *, fold: int, out_path: Path) -> None:
    ...
    print(
        f"[UNet-only ablation]{cv_note} model=models.unet.UNetSingle | input=[noisy] | data="
        f"{'pooled' if pooled else 'UNet-only ablation.data.OurDataDataset'} | band={cfg.band} T={cfg.segment_length} "
...
def main(argv: list[str] | None = None) -> None:
    ...
    p = argparse.ArgumentParser(
        description="UNet-only ablation: single-channel ablation (no DnCNN / no z; noisy-only input)."
    )
    ...
    print(f"[UNet-only ablation] checkpoint directory: {base_out_path.resolve()}", flush=True)

    run_cv_folds_with_workers(
        args,
        base_out_dir=base_out_path,
        cv_workers=int(args.cv_workers),
        log_prefix="[UNet-only ablation]",
    )
